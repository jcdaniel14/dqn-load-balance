from qlearning.utils import print_link,print_step
import numpy as np
import logging
import model
...
def get_training_links(test_id=0):
    if test_id == 0:
        return np.array(
            [
                {'congestionado': True, 'bw': 199, 'local': 'gye', 'capacidad': 200},
...
class QNEnv(object):
    ...
    def __init__(self, links=None , test_id=0):
        # === Entorno y estado inicial (Observation)
        self.links = get_training_links(test_id) if links is None else links
    ...
    def step(self, chosen_action):
        """
        - Recibe la accion a tomar por el qlearning, sea aleatoria u optimizada mediante la funcion Q
    ...
    def set_state(self, congested_link, chosen_link):
        """
        - Simula el movimiento de trafico hacia el enlace escogido por el qlearning
    ...
    def reset(self, links=None) -> list:
        ...
        """
        self.links = get_training_links(self.test_id) if links is None else links
        self.initial_state = self.get_current_state()
    ...
    def print_step_results(self,step,text_step):
        for idx, link in enumerate(self.links):
            print_step(link,idx, step, text_step)
